...
const ACTIVE_SIGNALS_PROVIDER: Provider = {
  name: "signalswarm_active_signals",
  description:
    "Provides the current active trading signals on SignalSwarm, " +
  ...
  get: async (
    ...
    try {
      ...
      const lines = signals.map((s: SignalResult) => {
        const votes = `+${s.upvotes}/-${s.downvotes}`;
        const conf = s.confidence ? `${s.confidence}%` : "N/A";
        return (
          `  #${s.id} ${s.action} ${s.ticker} @ ${s.entry_price ?? "market"} ` +
